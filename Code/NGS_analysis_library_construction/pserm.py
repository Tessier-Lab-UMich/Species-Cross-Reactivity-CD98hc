...
class ngs_analysis(object):
    ...
    def generate_MSA(self):
        ...
        for column in self.MSA.columns[:-1]:
            self.input_dict[column] = set(self.MSA.loc[:, column])
    ...
    def load_scores(self, fname, excel_or_csv = 'excel'):
        try:
            self.scores
    ...
    def score_all_samples_seq(self, inputs):
        ...
        if method == 'PSERM':
            dataset[seq] = [score_seq(self.PSERM[sample], seq) for sample in self.PSERM.keys()]
        
        elif method == 'PSSM':
            dataset[seq] = [score_seq(self.PSSM[sample], seq) for sample in self.PSSM.keys()]
    ...
    def cal_ER(self, lib1, lib2, out_dir):
        overlap_seqs = []
        new_seqs = []
        overlap_freq_seqs = {}
        new_freq_seqs = {}
        ...
        for seq in self.D.index:
            try:
                fo = self.D.loc[seq, lib1] 
                fi = self.D.loc[seq, lib2]
                pserm = self.scores.loc[seq, '%s PSERM Score' % lib2]
                if fo != 0 and fi != 0:
                    overlap_seqs.append(seq)
                    overlap_freq_seqs[seq] = [np.log2(fi / fo), fi, pserm]
                elif fi !=0 and fo == 0:
                    new_seqs.append(seq)
                    new_freq_seqs[seq] = fi
                else:
                    raise ValueError
            except:
                pass
        pd.DataFrame.from_dict(overlap_freq_seqs, orient = 'index', columns = [f'{lib2}_ER', f'{lib2}_FRE', f'{lib2}_PSERM_Score']).to_excel(os.path.join(out_dir, f'{lib2}_ER.xlsx'))
        pd.DataFrame.from_dict(new_freq_seqs, orient = 'index', columns = [f'{lib2}_FRE']).to_excel(os.path.join(out_dir, f'{lib2}_NEW_FRE.xlsx'))
        return overlap_freq_seqs, new_freq_seqs
